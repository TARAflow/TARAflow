...
> **Revision history**
> **v6** — Phase 5 splits into 5a (asset-anchored, new threats) and 5b (threat-anchored, feasibility into existing risk). Adds the anchor-driven two-use-cases model (§1), the `likelihoodSource` discriminator, and read-only impact/feasibility in the risk dialog (SSoT). Tab order fixed by the data flow, not deferred.
...
In Standard mode the same `threat`-anchored tree is used only on *uncertainty*, and
its output is **supplementary**: it informs the analyst, who then adjusts the OWASP
factors himself because he now understands the threat better. The tree does not
overwrite his assessment.
...
The method is recorded **per tree** and printed in the report: a quick-mode TARA is not audit-grade and must say so.
...
Formats: Markdown / AsciiDoc / HTML / PDF / StrictDoc.

---

## 9. Phase ordering (deferred)

For attack-tree-focused mode the natural tab order is `DFD → Assets → Threats → Attack Tree → Risks`. But correctness comes from the **data flow**, not tab position. Reordering touches `PhaseId`, `phaseStatus`, migration and the reporter — ergonomics only. **Data flow first; tabs last.**
...
### Phase 5b — Feasibility into the referenced risk, threat-anchored path
A `threat`-anchored tree emits **no** new threat. It writes its feasibility into
the *existing* risk of the threat it refines. This is the ISO normal case and the
Standard "refinement" case, and they differ only in `mode`:

```ts
interface Risk {
  // NEW — the discriminator. Without it, "empty factors" reads as likelihood 0
  // in calculateRiskValues: silent, wrong, dangerous direction.
  likelihoodSource: "factors" | "attack-tree";   // default "factors"

  attackTreeAssessment?: {
    treeId: string;
    pathKey: string;                              // the "AT-Branch" in the chain
    feasibilityBeforeMitigation: FeasibilityLevel;// from the tree, read-only
    mode: "supplementary" | "authoritative";
  };

  // The tree is a BEFORE-mitigation instrument (the close loop lives in the Risk
  // tab). Residual feasibility is assessed HERE, not derived from remaining paths.
  feasibilityAfterMitigation?: FeasibilityLevel;
}
```

| Case | Anchor | Method | `mode` | `likelihoodSource` | Factors | Impact shown |
|---|---|---|---|---|---|---|
| Refine | `threat` | Standard | `supplementary` | `factors` | editable | tree value shown, informs only |
| Refine | `threat` | ISO | `authoritative` | `attack-tree` | read-only, empty | from damage scenario (asset × goal) |
| New | `asset` | both | `authoritative` | `attack-tree` | read-only, empty | from asset |

`attackTreeAssessment` is written **whenever** a tree refines a threat, in both
modes — `mode` only decides whether the value *drives* the calculation
(`authoritative`) or *informs* the analyst (`supplementary`). It is persisted
either way: for the audit trail ("factors set by analyst, informed by attack-tree
feasibility Y") and because the chain (close loop, AT-Branch traceability) needs
`pathKey`/`treeId` on the risk regardless.

**Read-only in the risk dialog, editable only at the source (SSoT):**
- Feasibility → shown with provenance link to the Attack Tree tab
- Impact S/F/O/P → shown with provenance link to the Asset tab; the
  `ResolvedImpact.source` from Phase 3 says whether it is a goal override or the
  asset default. Never editable in the risk dialog — an edit there would fork the
  one impact that belongs to (asset × goal) and applies to every risk from it.

**close loop is unaffected.** `getLinkedElementsForAsset()` already resolves
asset → DFD element(s), so `Risk → threat → linkedAssetIds → element` is intact.
Phase 4 already sets `linkedAssetIds`; nothing to change, only verify.

**Tests (unit + integration):** ISO refine → `attack-tree`, factors read-only,
feasibility drives; Standard refine → `factors`, tree value stored but factors
drive; `calculateRiskValues` gives the right number for both sources; an existing
factor-based risk is byte-identical (default `likelihoodSource: "factors"`);
impact shown read-only with correct provenance.
**Commit:** `feat(risks): attack-tree feasibility as an alternate likelihood source`

### Phase 6 — Risks tab UI integration
Wire the model of 5a/5b into the dialog and table: hide/show the factor UI by
`likelihoodSource`, render the provenance links, the before/after mitigation split.
`getRisksByStrideMethod` and friends gain `"attack-path"`.
...
Per §8, including the mandatory methodology section.
...
### Phase 8 — UI polish (unblocked, parallel)
Split-handle drag (`editorWidthPercent` already persists — only the handle is missing), rename, guarded delete.
**Commit:** `feat(attacktree): resizable split view, rename and guarded delete`

### Phase 9 — Phase reordering (optional, last)

---

## 11. Open questions

1. **1:n damage scenarios** — revisit when a customer needs *separate risk treatment decisions per consequence* ("we accept the equipment damage but not the injury", both from one integrity violation). Until then the 1:1 simplification holds (§4). Trigger, not a deadline.
2. **EN 50742 vs. Annex G** — do the EN 50742 factors (WoO, AC, EL) map onto the ISO/IEC 18045 five, or are both sets needed side by side for EN-50742-scoped projects?
3. **Hazards toggle** — safety coupling purely impact-side (the S category), or does it also constrain feasibility?
