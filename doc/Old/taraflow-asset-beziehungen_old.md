...
Die Asset-Kategorien bilden eine Abstraktionshierarchie — je höher,
desto weniger Implementierungsdetails sind bekannt oder notwendig.
```
Data              ← niedrigste Ebene, höchste Präzision
  ↑ wird beeinflusst durch
Function          ← Fähigkeit / Capability
  ↑ mehrere Functions bilden
Process           ← konkrete Implementierung
  ↑ mehrere Processes bilden
System            ← Blackbox-Komponente
  ↑ mehrere Systems bilden
Infrastructure    ← höchste Ebene, geringste Präzision
```
...
|---------------------|----------------------|---------------------------------|
| Produktentwickler   | Function / Process   | "Set Brake Level" Prozess       |
...
sondern schneiden alle Ebenen:

| Kategorie      | Domäne    | Beispiele                                              |
|----------------|-----------|--------------------------------------------------------|
| **Physical**   | OT        | Motor, Ventil, Aktor, Sensor, Roboterarm               |
| **Service**    | Cloud/extern | Remote-Monitoring-Service, OTA-Update-Service, Vendor Cloud Backend, externe API |
| **Human**      | Personen  | Operator, Administrator, Angreifer, Schutzsubjekt      |
...
| Abstraktionsgrad      | Fähigkeit ("kann bremsen")        | Implementierung ("Set Brake Level") |
| `is_an` möglich?      | Ja (System → is_an → Function)    | Ja (Process → is_an → Process Asset) |

**Entscheidungsregel:**
> Wenn du den Implementierungsort kennst und im DFD modellieren kannst → **Process**.
> Wenn du nur das Verhalten kennst (Blackbox) → **Function**.
> Wenn eine Safety Function über mehrere Prozesse verteilt ist und als
> Einheit referenziert werden muss (z.B. via `externalRef` zur ISO 12100) → **Function**.

**Typische Anwendungsfälle:**

| Situation | Empfohlene Ebene |
|---|---|
| Eigenes Produkt, Quellcode bekannt | Process / Data |
| Fremdsystem, Dokumentation vorhanden | System / Data |
| Fremdsystem, nur Verhalten bekannt (Blackbox) | Function |
| Gesamtanlage, Integrator-Perspektive | Function / System |
| Safety Function verteilt über mehrere Prozesse | Function (zusätzlich zu Process) |

### Beispiel: Brake Control über alle Perspektiven
```
Produktentwickler:
  Process "Set Brake Level"
    └─ is_an → Process Asset "Set Brake Level"
    └─ modifies → Data Asset "Brake Configuration"

Integrator:
  System "Safety Controller"
    └─ is_an → System Asset "Safety Controller"
    └─ implements → Function Asset "Brake Control Function"

Betreiber:
  Infrastructure "OT Network Segment"
    └─ contains → System Asset "Safety Controller"
```

Gleicher Angriff, gleicher Schaden — unterschiedliche Modellierungstiefe.
