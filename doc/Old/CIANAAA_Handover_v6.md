...
**Version 6** — ThreatSource erweitert; Phase 2b (UnifiedStrategy) und Phase 3 (Risk-Tab) definiert
...
- ✅ Threat-Generierung (RelationStrategy mit CIANAAALevel) — implementiert
- ✅ Strategy Indicator im Threat Tab — implementiert
- ✅ ThreatSource erweitert (classic / hybrid / relation / manual) — implementiert
- 🔲 UnifiedStrategy Refactor — offen (Phase 2b)
- 🔲 Risk-Tab-Import-Modell — offen (Phase 3)
...
### Schritt 5 — Strategy Indicator im Threat Tab ✅

*Korrektur gegenüber v4: Eine "Threat Preview" im Asset Tab ist architektonisch falsch —
die Threat-Generierung findet ausschliesslich im Threat Tab statt. Der Asset Tab hat
keinen Schreibzugriff auf Threat-Daten.*

**Implementiert:** Der Threat Tab zeigt dem Analysten transparent welche Strategie aktiv
ist und warum sie gewählt wurde. Abgeleitet direkt aus `detectStrategy()` und
`computeAssetCoverage()`.

```
Aktive Strategie:  🔵 RelationStrategy
Grund:             Alle 12 Elemente haben verknüpfte Assets mit CIANAAA-Bewertung
                   → STRIDE wird aus Asset-Schutzzielen abgeleitet

── oder ──

Aktive Strategie:  🟡 HybridStrategy
Grund:             8/12 Elemente mit Assets · Projekt-Tags: IEC 62443
                   → STRIDE modifiziert durch Element-Properties und Projekt-Kontext

── oder ──

Aktive Strategie:  ⚪ ClassicStrategy
Grund:             Keine Assets verknüpft, keine Projekt-Tags gesetzt
                   → Generisches STRIDE per Element-Typ
```

Der Indicator kann auch einen manuellen Override anzeigen wenn
`configuration.strategyOverride` gesetzt ist.
...
```typescript
// shared/reference-types.ts (verschoben von asset-security-goals-types.ts)
...
## 4. Offene Punkte — Phase 2b: UnifiedStrategy Refactor 🔲

### Motivation

Die aktuelle Drei-Strategie-Architektur (Classic / Hybrid / Relation) ist künstlich:
Jede Strategie nutzt nur eine von drei verfügbaren Informationsquellen, die anderen
werden ignoriert. Ein Element kann gleichzeitig detaillierte Properties UND verknüpfte
Assets mit Schutzzielen haben — keine der drei Strategien nutzt beides.

### Drei Informationsquellen pro Element

| Quelle | Beispiel | Einfluss auf STRIDE |
|---|---|---|
| Element Properties | `protocol: "modbus"`, `encryptionInTransit: "tls"` | Modifikation (add/remove/deprioritize) |
| Asset Schutzziele | `Integrity=Critical`, `AuthN=High` | Ableitung via CIANAAA_TO_STRIDE + initialImpact |
| Keine Quelle | Element ohne Properties und Assets | Classic STRIDE |

### Ziel-Architektur: Additive Pipeline

Statt drei Strategien eine `UnifiedStrategy` mit optionalen Modulen:

```typescript
getStrideCategories(element, baseCategories, project): StrideCategory[] {
  let categories = [...baseCategories];

  // Module 1: Element Properties — immer angewandt wenn Properties vorhanden
  categories = applyElementProperties(element, categories);

  // Module 2: Asset Schutzziele — nur wenn active securityGoals vorhanden
  const goals = getActiveSecurityGoals(element.id, project);
  if (goals.length > 0) {
    categories = applyCIANAAA(goals, categories);
  }

  // Fallback: wenn beide Module keine Modifikation liefern → baseCategories
  return categories;
}
```

### ThreatSource — Erweiterung für UnifiedStrategy

`ThreatSource` muss ausdrücken durch welche Merkmale ein Threat generiert wurde:

```typescript
// Option A — granulare Flags
export interface ThreatGenerationModules {
  elementProperties: boolean;  // Module 1 aktiv
  cianaaa: boolean;            // Module 2 aktiv
}

export type ThreatSource =
  | "manual"
  | { type: "generated"; modules: ThreatGenerationModules };

// Option B — lesbare Kurzform (empfohlen für UI)
export type ThreatSource =
  | "manual"
  | "generated:classic"      // keine Modulation
  | "generated:properties"   // nur Element Properties
  | "generated:cianaaa"      // nur Asset Schutzziele
  | "generated:full";        // beides kombiniert
```

Option B ist UI-freundlicher und direkt als i18n-Key verwendbar.

### detectStrategy / strategyOverride

`detectStrategy()` wird obsolet — kein Coverage-Threshold mehr nötig.
`strategyOverride` bleibt als "force classic" Escape-Hatch für Spezialfälle
(z.B. schnelle Generierung ohne Asset-Lookup).

### Auswirkungen

- `strategy-factory.ts` — `detectStrategy` + `computeAssetCoverage` entfallen
- `classic-strategy.ts`, `hybrid-strategy.ts`, `relation-strategy.ts` — zusammengeführt
- `strategy-types.ts` — `IGeneratorStrategy` Interface vereinfacht
- `ThreatSource` — Typ angepasst (siehe oben)
- `StrategyIndicator` — zeigt aktive Module statt Strategie-Name
- Per-Threat Badge — direkt aus `source`-Feld ablesbar

### Schnittstellen für zukünftige Enrichment-Quellen

Die UnifiedStrategy-Pipeline muss so designed werden, dass zusätzliche
Enrichment-Quellen (Mitre ATT&CK, spezialisiertes LLM) nachträglich eingehängt
werden können — ohne die Pipeline-Architektur zu brechen.

Konkret: Ebene 1 (STRIDE-Ableitung) und Ebene 2 (Threat Enrichment) müssen
als separate Schichten implementiert werden. Siehe →
**TARAflow_ThreatEnrichment.md** für Konzept und geplante Phasen.

---

## 5. Offene Punkte — Phase 3: Risk-Tab-Import 🔲

### 4.1 Threat-Generierung: RelationStrategy mit CIANAAALevel ✅

**Implementiert in diesem Commit:**

- `CIANAAALevel`, `SecurityGoalType`, `SecurityGoalReference`, `CIANAAA_TO_STRIDE`
  nach `shared` verschoben — kein direkter Import von asset-feature in threat-feature
- `AssetReference` (shared) um `securityGoals?: SecurityGoalReference[]` erweitert
- `memoizedAssetDataRef` in `main-layout.tsx` befüllt `securityGoals` (nur active, `level !== "none"`)
- `Threat` um `initialImpact?: CIANAAALevel` erweitert
- `IGeneratorStrategy` um `getInitialImpact()` erweitert
- `RelationStrategy` komplett neu: liest `project.assetDataRef` statt `RELATION_TO_STRIDE`,
  leitet STRIDE via `CIANAAA_TO_STRIDE` ab, setzt `initialImpact` via `MAX(level)`
- `ClassicStrategy` / `HybridStrategy`: `getInitialImpact()` gibt `undefined` zurück
- `element-generator.ts` + `interaction-generator.ts`: setzen `threat.initialImpact`

**Datenfluss:**
```
Asset Feature (a.securityGoals)
  → memoizedAssetDataRef (SecurityGoalReference[])
    → ThreatProjectData.assetDataRef
      → RelationStrategy.getLinkedAssets()
        → getStrideCategories() → STRIDE Union
        → getInitialImpact()    → MAX(CIANAAALevel)
          → threat.initialImpact
```

### 4.2 Risk-Tab-Import-Modell 🔲 (Phase 3)
...
## 6. Abgrenzung: Was NICHT geändert wurde / wird
...
## 7. Für den nächsten Chat

**Nächstes Thema:** Phase 2b — UnifiedStrategy Refactor

Danach: Phase 3 — Risk-Tab-Import
