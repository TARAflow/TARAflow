...
EE "Pflegepersonal"
└─ accesses [local] → Infrastructure Asset "Patientenzimmer (physischer Zugang)"
...
> **High-Value Asset:** Die Dockingstation ist als `isHighValueAsset` klassifiziert.
> `assetDestructionImpact: 'high'` löst keinen automatischen Override aus, erhöht
> aber die Threat-Priorität gegenüber Standard-HIGH Assets.
...
| Feld | Inhalt |
|---|---|
| **DFD-Element** | EE `Wartungstechniker` via `accesses [local] → Dockingstation` |
