...
kein logisches Konstrukt, sondern ein konkreter physischer Perimeter (Gehäuse, Schaltschrank,
Raum, Gebäude). Sie ist connectable — DataFlows können an ihr terminieren via Interface-Symbol
auf der Boundary-Kante.

Semantisch klar getrennt von:
- `TrustBoundary` — beschreibt logische Vertrauensgrenzen (Wer vertraut wem?)
- `ChipBoundary` — beschreibt Hardware-/Execution-Isolation (Welche HW-Isolation existiert?)
- `PhysicalBoundary` — beschreibt räumliche Zugriffsbarrieren (Wer kommt physisch ran?)
 
### Relevante Asset-Beziehungen
 
| Relation | Beispiel | Threat-Relevanz |
|---|---|---|
| `is_an` (Physical) | Schaltschrank PhysicalBoundary `is_an` Physical Asset "Schaltschrank" | Instanz-Beziehung: Boundary ist das Asset selbst |
| `is_an` (Infra) | Serverraum PhysicalBoundary `is_an` Infrastructure Asset "Serverraum" | Instanz-Beziehung: Boundary ist Infrastruktur-Asset |
| `secures` (Physical) | Gehäuse PhysicalBoundary `secures` Physical Asset "Prototyp" | Strukturelle Beziehung: Boundary schützt physischen Asset |
| `secures` (Infra) | Schaltschrank PhysicalBoundary `secures` Infrastructure Asset "Feldbus-Switch" | Boundary schützt Infrastruktur-Asset |
| `accesses` (Physical) | Attacker ExternalEntity `accesses` PhysicalBoundary | Angriffsvektor: Unauthorized Physical Access |
| `damages` (Physical) | Saboteur ExternalEntity `damages` PhysicalBoundary | Impact: Physische Zerstörung der Barrier |
| `depends_on` (System) | Safety PLC Multiprocess `depends_on` PhysicalBoundary | Impact: Ausfall physischer Schutzbarriere |
| `powers` (Infra) | UPS Infrastructure `powers` PhysicalBoundary | Verfügbarkeits-Abhängigkeit der Boundary |
 
### Erlaubte Relationen je Asset-Gruppe

| Asset-Gruppe | Erlaubte Relationen | Begründung |
|---|---|---|
| Data | — | PB erzeugt/liest keine Daten direkt |
| Function | — | PB führt keine Funktionen aus |
| Process | — | PB orchestriert keine Prozesse |
| System | `is_an`, `depends_on` | PB kann eine System-Instanz sein oder davon abhängen |
| Infrastructure | `is_an`, `secures`, `powers` | PB ist oft eine Infrastruktur-Ressource |
| Physical | `is_an`, `secures`, `accesses`, `damages` | Kernrelation: PB ist selbst ein Physical Asset |
| Service | — | PB konsumiert keine Services |
| Human | — | Auswirkungen auf Menschen laufen via innere Assets |

### Verbotene Verbindungen (Validator)
 
DataFlow zwischen zwei PhysicalBoundaries ist nicht erlaubt — Daten fliessen durch
Boundaries, nicht zwischen ihnen. DataFlows terminieren an einer PhysicalBoundary
über ein Interface-Symbol auf der Boundary-Kante (analog zu ChipBoundary).
 
### Safety-Relevanz
 
PhysicalBoundaries können safety-relevante Bereiche umschliessen:
 
- Schaltschrank mit Safety-PLC: `safetyRelevant = true`
- Gehäuse eines SIL-zertifizierten Geräts: `safetyRelevant = true`
- Standard IT-Serverraum: `safetyRelevant = false`

`PhysicalBoundaryProperties.safetyRelevant + safetyRationale` erlaubt automatische
Priorisierung von Threats auf safety-kritischen physischen Perimetern (z.B. Unauthorized
Access zu Safety PLC → höchste Priorität via Safety Override Rule).
