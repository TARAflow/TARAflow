...
| **Physical**   | Passiver Sachwert ohne eingebettetes System | Museumsgemälde, Prototyp, physischer Schlüssel |
...
**Entscheidungsregel:**
> Ist das Asset eine **Fähigkeit** die ein System haben muss? → **Function**.
> Ist das Asset eine **Abfolge von Schritten** deren Reihenfolge und Kombination den Wert ausmacht? → **Process**.
> Ist es ein einzelner Software-Ablauf dessen Implementierung bekannt ist? → Kein eigenes Asset nötig — im DFD als Process-Element modellieren.

**Typische Anwendungsfälle:**

| Situation | Empfohlene Kategorie |
|---|---|
| Safety-Anforderung aus ISO 26262 / MDR / ISO 12100 | Function |
| Fertigungsrezept / GMP-Herstellungsvorschrift | Process |
| Behandlungsprotokoll (klinisch) | Process |
| NC-Programm / Fräsrezept | Process |
| Bremsfunktion als Blackbox | Function |
| Firmware-Algorithmus (Quellcode bekannt) | Im DFD modellieren, kein Process Asset |
...
Beschreibt **betriebliche Abläufe und Knowhow-Assets** — die zeitliche Sequenz von Schritten
die ein Ergebnis erzeugen. Der Wert des Assets liegt nicht in den einzelnen Bestandteilen,
sondern in deren **Reihenfolge und Kombination**.

Process Assets sind **orthogonal** — sie orchestrieren alle anderen Asset-Kategorien über die
Zeit hinweg: sie nutzen Data, rufen Functions auf, laufen auf Systems und werden von Humans
bedient.

**Typische Beispiele:**
- Fertigungsrezept / NC-Programm (Maschinenbau)
- GMP-Herstellungsvorschrift für Wirkstoffe (Pharma)
- Klinisches Behandlungsprotokoll (Medical)
- Zertifiziertes Schweissverfahren (Anlagenbau)
- Dialyse-Behandlungsprotokoll (Medical Device)

**Threat-Fokus:**
- **Confidentiality:** IP-Diebstahl — der Angreifer stiehlt nicht Daten oder Funktionen
  (die sind isoliert wertlos), sondern das Rezept als Ganzes
- **Integrity:** Sequenzmanipulation — einzelne Steps korrekt, Reihenfolge geändert
  → Qualitätssabotage ohne sichtbaren Einzelfehler
- **Availability:** Verlust des Prozess-Knowhows → Produktionsstopp + Rezertifizierungsaufwand

### DFD-Einstieg (Ebene 1)

Process Assets haben DFD-Einstiegspunkte — ein DFD-Element kann einen Prozess starten,
überwachen oder beenden:

```
executes     - DFD-Element führt den Prozess aus (startet und steuert die Sequenz)
...
monitors     - DFD-Element überwacht den Prozess-Fortschritt
is_an        - DFD-Element ist eine Instanz dieses Prozesses (selten)
```
...
**Schritt-Beziehungen (geordnet):**
Die Kernbeziehungen die die Sequenz definieren. Sie tragen einen optionalen `stepOrder`-Qualifier:
```
...
**Ressourcen-Beziehungen (ungeordnet):**
Beschreiben womit und durch wen der Prozess ausgeführt wird — keine Reihenfolge:
```
...
```
Process Asset "Turbinenschaufel-Fräsrezept"
├─ [step: 1] invokes → Function Asset "Schruppen"
│              uses  → Data Asset "Schnittparameter-Schruppen"
├─ [step: 2] invokes → Function Asset "Schlichten"
│              uses  → Data Asset "Oberflächentoleranz"
├─ [step: 3] invokes → Function Asset "Qualitätsprüfung"
│              uses  → Data Asset "Abnahmekriterien"
├─ runs_on   → System Asset "5-Achs CNC-Steuerung"
└─ operated_by → Human Asset "CNC-Einrichter"

→ Threat Sequencing Attack: Schritt 2 und 3 vertauscht
  → Qualitätssabotage: Schlichtvorgaben ohne vorherige Rauheitsüberprüfung
  → Kein einzelnes Asset kompromittiert — nur die Sequenz

Process Asset "Dialyse-Behandlungsprotokoll"
├─ [step: 1] invokes → Function Asset "Heparin-Bolus verabreichen"
├─ [step: 2] invokes → Function Asset "Blutpumpe starten"
├─ [step: 3] invokes → Function Asset "Filtration überwachen"
├─ [step: 4] invokes → Function Asset "Blut rückführen"
├─ uses → Data Asset "Patientenspezifische Parameter"
└─ operated_by → Human Asset "Dialysepflegefachkraft"

→ Threat Sequencing Attack: Schritt 4 vor Schritt 3
...
interface ProcessAssetProperties {
  ...
  // Klassifikation
  domain?: string;                    // z.B. "GMP", "OT-Manufacturing", "Medical"
  regulatoryReference?: string;       // z.B. "GMP Annex 11", "MDR Annex I"
  isValidatedProcess?: boolean;       // Zertifizierter/validierter Prozess?
  ...
  // IP-Schutz
  isProprietaryKnowHow?: boolean;     // Schützenswertes Betriebsgeheimnis?
  knowHowRationale?: string;          // Pflicht wenn isProprietaryKnowHow === true
}
```
...
> - **depends_on:** Definiert eine kritische Verfügbarkeits-Abhängigkeit. Fällt das System-Asset aus oder wird es kompromittiert, resultiert daraus ein **Totalausfall (Availability-Impact)** des abhängigen DFD-Elements.
...
```txt
local      - Direkter Zugriff vor Ort (z.B. Werksgelände)
...
└─ accesses [local] → Infrastructure Asset "CNC Machine"
...
- `highValueRationale`: Pflichtfeld wenn `isHighValueAsset === true`.
...
```
IF isHighValueAsset === 'critical' OR isHighValueAsset === 'high'
THEN Threat-Priorität = CRITICAL (unabhängig vom Likelihood-Score)
...
THEN Threat-Priorität = HIGH minimum (unabhängig vom Likelihood-Score)
...
**Operational Criticality Override Rule:**
```
IF operational === 'critical'
AND replacementLeadTime >= '3-6m (medium)'
THEN Threat-Priorität = HIGH minimum (unabhängig vom Likelihood-Score)
     → verhindert dass betriebskritische Assets auf LOW fallen
     → kein volles CRITICAL — Safety Override bleibt stärker
```

**Rangfolge der Override Rules (absteigend):**

1. Safety Override      — fatality/irreversible_injury → CRITICAL
2. HVA critical/high    → CRITICAL
3. HVA medium           → HIGH minimum
4. HVA low              → kein Override
...
└─ Properties: {
     ...
     replacementLeadTimeNote: 'Lead time >12m but parallel unit in fab B available
                               as fallback → assetDestructionImpact downgraded to high',
...
- `Physical → Function` (`executes`, `triggers`)
...
interface PhysicalAssetProperties {
  ...
  // High-Value Extension (analog zu Infrastructure Assets)
...
| DFD-Einstieg | ✅ via `accesses` Element-zu-Asset | ❌ kein DFD-Einstieg |
...
Service Assets sind Schnittstellen zu Akteuren oder Infrastrukturen ausserhalb des eigenen
Systemgrenzen — sie sind im DFD nicht als eigenes Element modellierbar.
...
> - `depends_on` (Hard-Dependency): Ausfall des Service → Totalausfall des DFD-Elements
...
interface ServiceAssetProperties {
  ...
  responsibilityScope?: string;   // Pflicht wenn responsibility === "shared"
...
THEN Validierungswarnung:
     "responsibilityScope ist Pflichtfeld bei Shared Responsibility
...
- **Abgeleitete Felder auf AssetProperties (Strict Default, Override möglich):**
  ...
  - `nonRepudiationRelevant`, `authenticationRelevant`, `authorizationRelevant`, `accountabilityRelevant` — abgeleitet aus Relationstypen (Tabelle Abschnitt 4.2)
...
- `nonRepudiationRelevant` + `nonRepudiationSource` + `nonRepudiationRationale`
- `authenticationRelevant` + `authenticationSource` + `authenticationRationale`
- `authorizationRelevant` + `authorizationSource` + `authorizationRationale`
- `accountabilityRelevant` + `accountabilitySource` + `accountabilityRationale`
...
- Derivation-Engine: Relationstypen → NAAA-Flags (Tabelle Abschnitt 4.2)
