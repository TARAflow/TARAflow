...
### Hazard Item — der explizite Gefährdungsknoten

Ein **Hazard Item** ist ein eigener Knotentyp auf einer orthogonalen Ebene
(neben Physical, Process, Service, Human, Environment). Es ist **kein Asset** —
ein Asset ist ein schützenswertes Gut, ein Hazard Item ist die Gefährdung selbst.
Das Hazard Item ist der **Konvergenzpunkt**, an dem ein oder mehrere
gefährdungsverursachende Assets zusammenlaufen und von dem aus ein oder mehrere
Schutzziele gefährdet werden.

```txt
Asset ─contributes_to [relevance]─► Hazard Item ─endangers [impact]─► Schutzziel
```

**Zwei Kantentypen, sauber getrennt:**

| Kante | Von → Nach | Trägt | Bedeutung |
|---|---|---|---|
| `contributes_to` | Asset → Hazard Item | `relevance` (direct/indirect) | Steuert/ermöglicht dieses Asset die physische Aktion, die das Hazard ausmacht? |
| `endangers` | Hazard Item → Schutzziel | `impact` | Welcher Schaden droht dem Schutzziel? |

**Das Hazard Item trägt** die ISO 12100-Information und das Gefährdungspotenzial:

```typescript
type HazardItem = {
  id: string;
  label: string;                  // z.B. "exotherme Reaktion", "Kollision/Quetschung"
  hazardType?: string;            // ISO 12100 Gefährdungsart (mechanisch, thermisch, chemisch, ...)
  physicalHazardPotential?: "low" | "medium" | "high";  // 3-stufig nach ISO 12100
  combinationType: "ANY" | "ALL"; // wie mehrere contributes_to-Eingänge zusammenwirken
  externalRef?: string;           // Verknüpfung zur externen Safety-Analyse (z.B. "HAZ-007")
  rationale?: string;
};
```

**`combinationType`** entscheidet die Logik mehrerer Eingänge:
...
> Mehrere `contributes_to`-Eingänge implizieren **nicht** automatisch Kombinatorik.
> Ob es sich um unabhängige Ursachen (ANY) oder eine gemeinsame Bedingung (ALL)
> handelt, wird explizit über `combinationType` festgelegt. In FTA-Begriffen:
> ANY = OR-Gate, ALL = AND-Gate.

**Schutzziel-Ableitung:** Die Impact-Dimension der `endangers`-Kante wird aus dem
Zieltyp abgeleitet — kein expliziter Kantentyp pro Dimension nötig:

| Zielknoten | abgeleitete Dimension | Impact-Skala / Norm |
|---|---|---|
| Human | Safety | reversible · irreversible · fatality (ISO 12100) |
| Environment | Umwelt | minor · moderate · severe (Seveso / Umwelthaftung) |
| Infrastructure | Asset-Zerstörung | minor · major · critical |

```typescript
type HazardImpact =
  | { target: "human";          severity: "reversible" | "irreversible" | "fatality" }
  | { target: "environment";    severity: "minor" | "moderate" | "severe" }
  | { target: "infrastructure"; severity: "minor" | "major" | "critical" };
```

**Transducer als bewusste Ausnahme — direkte Wirkung auf Schutzziel-Kategorien.**
Grundsätzlich werden Schutzziele (Human, Environment, Infrastructure) über die
Hazard-Item-Kette erreicht: `Asset ─contributes_to─► Hazard Item ─endangers─► Schutzziel`.
Sensoren und Actuatoren sind cyber-physische Wirkelemente und tragen **zusätzlich**
eine direkte Element→Asset-Relation auf die betroffene Kategorie:

- **Sensor** — `monitors` Environment, `affects_privacy`/`identifies`/`tracks` Human
  (reine Beobachtung, *kein* Hazard).
- **Actuator** — `endangers` Human, `endangers`/`contaminates` Environment,
  `damages` Physical/Infrastructure (die Wirkung *ist* die Gefährdung).

Diese direkte Relation ist der **Schnell-Pfad** für den Ein-Ursachen-Fall. Für
mehrstufige oder kombinatorische Gefährdungen (mehrere `contributes_to`-Eingänge,
ANY/ALL) wird **zusätzlich** ein Hazard Item als Konvergenzpunkt modelliert. Beide
Darstellungen koexistieren — die direkte Relation für den einfachen Fall, das
Hazard Item für die explizite Bowtie-Kombinatorik. Entsprechend liefert
`getAllowedRelations` für `environment` neu die Transducer-Relationen (statt `[]`);
für alle übrigen Elementtypen bleibt Environment ausschliesslich Schutzziel.

#### Darstellung: immer im Modell, optional sichtbar

Das Hazard Item existiert **immer im Datenmodell** — aber nicht immer sichtbar.
Dies entspricht dem Prinzip der Worst-Case-Hazardous-Situation (siehe oben):
konzeptuell immer vorhanden, nicht immer als Feld ausgeprägt.

**Eingeklappt** (Default bei genau einem Eingang mit ableitbarer Gefährdung):

```txt
Robot Arm ──endangers──► Einrichter
```

Der Analyst zeichnet die direkte Kante. Das Tool legt intern automatisch
ein Hazard Item an (ableitbar aus Asset-Typ und Kontext, z.B. "Kollision/Quetschung").
`relevance` und `impact` werden auf die implizite Struktur gemappt.

**Aufgeklappt** (automatisch bei mehreren Eingängen — Kombinatorik muss sichtbar sein):

```txt
Natrium-Dosierventil ─┐
                       ├─contributes_to─► Hazard "exotherme Reaktion" ─endangers─► Bediener
Wasser-Einlassventil ─┘
```

Sobald ein zweiter `contributes_to` auf dasselbe Hazard Item zeigt, klappt es auf.
Bei `combinationType: 'ALL'` entsteht ein **AND** — die Beiträge konvergieren als
gemeinsame Bedingung auf denselben Gefährdungsknoten. Bei `combinationType: 'ANY'`
sind es unabhängige Ursachen (OR). Kein expliziter Boolean-Operator auf den Kanten,
keine Fault-Tree-Algebra — die Logik sitzt als ein Feld am Hazard Item.
(Auditoren, die FTA kennen: ALL entspricht einem AND-Gate, ANY einem OR-Gate.)

**Regeln:**
- Ein Eingang + ableitbares Hazard → eingeklappt darstellbar
- Mehrere Eingänge → immer aufgeklappt
- `relevance` lebt auf `contributes_to`, `impact` auf `endangers`
- `physicalHazardPotential` + ISO 12100-Felder leben am Hazard Item

#### Beispiel: kombinatorische Gefährdung (Natrium/Wasser)

Weder das Natrium-Dosierventil noch das Wasser-Einlassventil allein verursachen
die Katastrophe. Erst die **gemeinsame Übersteuerung** beider Ventile bringt
Natrium und Wasser zusammen — die exotherme Reaktion entsteht.

```txt
Physical "Natrium-Dosierventil" ─contributes_to [direct]─┐
                                                          │
                                  Hazard "Natrium-Wasser-Reaktion"
                                    hazardType: chemisch/thermisch
                                    physicalHazardPotential: high
                                    combinationType: ALL
                                                          │
Physical "Wasser-Einlassventil" ─contributes_to [direct]─┘
                                                          │
                          ┌───────────────────────────────┼───────────────────────────┐
                          ▼                                ▼                           ▼
              endangers [fatality]              endangers [severe]          endangers [critical]
                          │                                │                           │
                    Human "Bediener"          Environment "Gewässer"   Infrastructure "Produktionsanlage"
```

Die Ventile tragen `relevance: direct` — sie steuern unmittelbar die physische
Aktion (Flussrate). Die Tanks selbst sind passive Behälter und nicht der
`contributes_to`-Träger; ihr Gefahrenpotenzial wird über das Hazard Item
(`physicalHazardPotential: high`) abgebildet.
...
Sie wird als **Hazard Item** modelliert (siehe Abschnitt "Hazard Item"),
das im einfachsten Fall (ein Eingang, ableitbare Gefährdung) eingeklappt bleibt
und nicht als eigenes Feld ausgeprägt werden muss.
...
**relevance: `indirect`**\
Das Asset kontrolliert die gefährliche physische Aktion nicht unmittelbar.
`indirect` hat **zwei Ursachen**, die funktional verschieden sind:

*1. Systemischer Einfluss* — das Asset beeinflusst nur:
  - Verfügbarkeit von Systemen oder Netzwerken
  - Konfiguration oder Parametrierung
  - Infrastruktur oder Übertragungswege
  - Zugang oder Authentifizierung
  → Kompromittierung = systemischer Einfluss, kein direkter Hebel
  (z.B. Netzwerk-Switch, Engineering-Workstation, Active Directory)

*2. Gefährdungsträger ohne Steuerungsfunktion* — das Asset enthält die Gefahr
  (Stoff, Energie), steuert sie aber nicht aktiv:
  - passiver Behälter (Chlortank, Natriumtank, Druckbehälter)
  - Energiespeicher (Batteriesystem)
  → Kompromittierung allein steuert nichts — die Gefahr liegt im Asset,
  die Auslösung erfolgt über ein steuerndes Asset (Ventil, Controller)

> **Unterscheidung über `physicalHazardPotential`:** Beide Klassen sind formal
> `indirect`, aber analytisch verschieden. Ein Netzwerk-Switch ist
> `indirect` + `physicalHazardPotential: low`; ein Natriumtank ist
> `indirect` + `physicalHazardPotential: high`. Erst diese zweite Dimension
> macht den Unterschied im Modell sichtbar.

> **`relevance` ist funktional, nicht topologisch.** Die Position in einer Kette
> ändert `relevance` nicht. Mehrere Glieder können gleichzeitig `direct` sein, wenn
> jedes die physische Aktion unmittelbar mitsteuert:
> ```
> Ventil           relevance: direct    (steuert Flussrate unmittelbar)
> PLC              relevance: direct    (steuert das Ventil unmittelbar)
> Engineering Stn. relevance: indirect  (konfiguriert nur)
> ```
> Die *topologische* Distanz zum Hazard wird separat über `hazardDistance` auf der
> `contributes_to`-Kante getragen (1, 2, 3...). Das Hop-Limit (siehe Asset-zu-Asset-
> Dokument, Regel 2) lebt auf `hazardDistance` — nicht auf `relevance`. So wird
> `relevance` nie still topologisch umgedeutet.
...
## Sensor — Asset-Beziehungen
 
### Konzept
 
Ein Sensor ist ein Transducer **physisch → cyber**: er wandelt eine physikalische
Grösse in ein Signal. Er ist connectable — die **Cyber-Seite** terminiert via
DataFlow an einem Process/Interface (der Messwert), die **physische Seite** an
einer ExternalEntity (der physikalischen Umwelt) über einen DataFlow mit
`medium="physical"`. Threat-Fokus: **Integrität / Authentizität / Verfügbarkeit
des Messwerts**.
 
Aktive Sensoren (Radar/LiDAR/Ultraschall) werden **nicht** hier als Flag geführt,
sondern als Dual-Role-Element modelliert (Sensor + emission-Actuator).
 
> Modellierungsfrage „Kamera = Sensor oder ExternalEntity?": eine Kamera als
> Messelement (optischer Transducer) ist legitim ein **Sensor** — damit greifen
> Privacy-Beziehungen zu Human Assets (siehe unten).
 
### Relevante Asset-Beziehungen
 
| Relation | Beispiel | Threat-Relevanz |
|---|---|---|
| `creates` (data) | Drehzahlsensor `creates` Messwert „Speed" Data Asset | Spoofing/Injection des Messwerts |
| `accesses` (physical, indirect) | Kamera-Sensor `accesses` „Fahrbahn" Physical Asset | Optische Injection / Blinding |
| `monitors` (physical) | Temperatursensor `monitors` „Reaktorbehälter" Physical Asset | Unterdrückter Alarm via gefälschtem Wert |
| `monitors` (environment) | Luftgütesensor `monitors` „Umgebungsluft" Environment Asset | Verfälschte Umweltdaten |
| `affects_privacy` / `identifies` / `tracks` (human) | Kamera-Sensor `tracks` „Person" Human Asset | Überwachung / Privacy-Verletzung |
| `implements` / `monitors` (function) | Sensor `implements` „Geschwindigkeitsmessung" Function Asset | Funktions-Degradation |
| `is_an` (system) | MEMS-IMU `is_an` „Inertial Measurement Unit" System Asset | Instanz-Beziehung |
| `depends_on` (system) | Sensor `depends_on` „Sensor-Versorgung" System Asset | DoS via Stromausfall |
 
**Hauptbeziehungen nach Asset-Kategorie:**
 
```
Physical Asset     → accesses (indirect) / monitors   (misst/beobachtet die Grösse)
Data Asset         → creates                          (erzeugt den Messwert)
Environment Asset  → monitors                         (beobachtet Umweltzustand)
Human Asset        → affects_privacy / identifies / tracks   (überwachende Sensoren)
System Asset       → is_an / depends_on               (Gerät; Versorgung/Bus)
Function Asset     → implements / monitors            (realisiert die Messfunktion)
```
 
### Verbotene Verbindungen (Validator R11)
 
- Die **physische Seite** eines Sensors terminiert ausschliesslich an einer
  ExternalEntity über einen DataFlow mit `medium="physical"` — nicht an einem
  Process, DataStore oder einer Boundary.
- Die **Cyber-Seite** terminiert an Process oder Interface.
- Ein Sensor darf **nicht** direkt mit einem DataStore verbunden werden (der
  Messwert wird als Data Asset bzw. nachgelagerter Process/DataStore modelliert).
- `damages` / `secures` / `endangers` sind für Sensoren unzulässig — ein Sensor
  *beobachtet*, er *wirkt nicht* (das ist die Actuator-Seite).
### Safety-Relevanz
 
Ein Sensor ist die **Input-Seite des Bowtie**: ein gefälschter oder unterdrückter
Messwert kann eine Safety-Funktion aushebeln (defeated barrier).
 
- `safetyRelevant = true`, wenn ein korrupter Messwert eine Gefährdung treiben kann
  (z. B. zu niedrig gemeldete Temperatur → ausbleibende Abschaltung).
- `safetyClassification` nach Konsequenz-Magnitude (standard-agnostisch, projiziert
  auf SIL/ASIL/Klasse).
- Mitigations-Properties dämpfen das Threat-Set: `signalAuthentication`,
  `plausibilityCheck`, `redundancy` (nur `diverse` gegen Common-Mode), `lossDetection`.
- `safetyRelevant = undefined` ⇒ Validator-Finding (unbewertet), **nicht** stilles
  Unterdrücken.
---
 
## Actuator — Asset-Beziehungen
 
### Konzept
 
Ein Actuator ist ein Transducer **cyber → physisch**: er wandelt einen Befehl in
eine physikalische Wirkung. Er ist connectable — die **Cyber-Seite** empfängt via
DataFlow von einem Process/Interface (das Kommando), die **physische Seite** wirkt
auf eine ExternalEntity (die kontrollierte Umwelt) über einen DataFlow mit
`medium="physical"`. Threat-Fokus: **Integrität / Authentizität des Kommandos** und
**Verfügbarkeit der Safe-State-Funktion**. Ein Actuator ist typischerweise das
**Top-Event des Bowtie**.
 
### Relevante Asset-Beziehungen
 
| Relation | Beispiel | Threat-Relevanz |
|---|---|---|
| `reads` (data) | Ventil-Actuator `reads` „Auf/Zu-Kommando" Data Asset | Gefälschtes Kommando |
| `accesses` (physical, direct) | Roboterarm `accesses` „Werkstück" Physical Asset | Erzwungene Bewegung / Quetschung |
| `damages` (physical) | Motor-Actuator `damages` „Förderband" Physical Asset | Zerstörerischer Over-Drive |
| `secures` (physical) | Schloss-Actuator `secures` „Zugangstür" Physical Asset | Unautorisiertes Entriegeln |
| `endangers` (human) | Roboterarm `endangers` „Bediener" Human Asset | Physische Verletzung (Hazard-Top-Event) |
| `endangers` / `contaminates` (environment) | Dosierpumpe `contaminates` „Prozesswasser" Environment Asset | Überdosierung / Leckage |
| `implements` (function) | Actuator `implements` „Not-Halt" Function Asset | Ausfall der Schutzfunktion |
| `is_an` (system) | Servoantrieb `is_an` „Drive Unit" System Asset | Instanz-Beziehung |
| `depends_on` (system) | Actuator `depends_on` „Stromversorgung" System Asset | DoS → Verlust Safe-State |
| `damages` (infrastructure) | Pumpe `damages` „Rohrleitung" Infrastructure Asset | Überdruck |
 
**Hauptbeziehungen nach Asset-Kategorie:**
 
```
Physical Asset     → accesses (direct) / damages / secures   (wirkt/zerstört/sichert)
Human Asset        → endangers                               (Hazard-Top-Event)
Environment Asset  → endangers / contaminates                (Emission/Flow/Dispensing)
Data Asset         → reads                                   (konsumiert das Kommando)
System Asset       → is_an / depends_on                      (Gerät; Versorgung/Controller)
Function Asset     → implements                              (realisiert die Aktuierung)
Infrastructure     → damages                                 (z. B. Überdruck)
```
 
> `damages` ist hier die **legitime Hazard-Relation** (Konsequenz einer
> Fehl-/Missbrauchs-Aktuierung), nicht die Sabotage-Ausnahme wie bei ExternalEntity.
 
### Verbotene Verbindungen (Validator R12)
 
- Die **physische Seite** eines Actuators terminiert ausschliesslich an einer
  ExternalEntity über einen DataFlow mit `medium="physical"`.
- Die **Cyber-Seite** empfängt von Process oder Interface (das Kommando).
- Ein Actuator darf **nicht** direkt mit einem DataStore verbunden werden.
- `creates` (data) ist unzulässig — ein Actuator *erzeugt* keinen Messwert. Eine
  Rückmeldung (Feedback) ist die **Sensor-Seite** eines bidirektionalen Elements,
  nicht der Actuator selbst.
### Safety-Relevanz
 
Der Actuator ist meist das **Top-Event**: erzwungene, blockierte oder ausbleibende
Aktuierung *ist* die Gefährdung.
 
- `hazardPotential` (intrinsisch, kontextfrei — Lüfter vs. Roboterarm) seedet die
  Priorisierung; getrennt von der bewerteten `safetyClassification`.
- Zentrale Safety-Properties: `safeState`, `failBehavior`, `feedbackVerification`,
  `hardwareInterlock`. **De-Energize ist nicht automatisch sicher** (Bremse löst beim
  Stromausfall) — `safeState` immer zusammen mit `actuatorClass` lesen.
- `endangers` (human) / `endangers`|`contaminates` (environment) ⇒ `safetyRelevant = true`.
- `hardwareInterlock = "independent"` ist der mechanische Pfad, der eine
  Controller-Kompromittierung überlebt — bounding-Wert für den Worst Case.
...
```txt
endangers         - Hazard Item beeinflusst physische Sicherheit
affects_safety    - DEPRECATED (Legacy). Ersetzt durch `endangers` (eingeführt mit
                    dem Hazard Item). `endangers` ist die kanonische Safety-Relation
                    auf Human-/Environment-Schutzziele. `affects_safety` bleibt vorerst
                    nur rückwärtskompatibel erhalten; neue Modelle verwenden `endangers`.
