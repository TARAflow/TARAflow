...
Zur konsistenten Modellierung von Data Flows innerhalb des DFD wird ein **verbindliches Naming-Konzept** angewendet. Dies dient der **Eindeutigkeit, Nachvollziehbarkeit und Automatisierung** der Bedrohungsanalyse. Durch die klare Trennung von Kommunikationsrichtung, Semantik und Zieltyp können Threats systematisch abgeleitet und Tool-Unterstützung (z. B. automatisches Linting und STRIDE-Mapping) effektiv genutzt werden.

Das Verb kodiert die **Interaktionssemantik**, nicht die Perspektive von Sender oder Empfänger. Beide Richtungen eines Request/Response-Paars verwenden dasselbe Verb — weil die Art der Interaktion beschrieben wird, nicht die Sichtweise.
...
- **Verb**  
  Definiert die Interaktionssemantik. Es gibt genau vier gültige Verben:

  | Verb | Bedeutung | Ziel | Flow-Type |
  |------|-----------|------|-----------|
  | `pull` | Explizite Anforderung mit erwarteter Antwort | Process oder Data Store | `[req]` / `[resp]` (Pflicht) |
  | `push` | Einwegübertragung ohne Antwort (fire-and-forget) | Process | `[cmd]` (Pflicht) |
  | `write` | Einwegschreiben in einen Data Store | Data Store | keiner (weggelassen) |
  | `stream` | Kontinuierlicher Datenfluss | Process | `[stream]` (optional) |

  > **Deprecated:** `send` und `recv` sind nicht mehr gültig.  
  > `send` wird durch `push` ersetzt. `recv` entfällt — der Response-Flow ist bereits ein eigener `pull [resp]`.

- **Object**  
  Beschreibt das transportierte Asset oder die payload-spezifische Entität (z. B. `nc program`, `safety params`, `machine status`).

- **Flow-Type**  
  Pflichtfeld für `pull` und `push`, nicht verwendet bei `write`:
  - `[req]` — Request (nur bei `pull`)
  - `[resp]` — Response (nur bei `pull`)
  - `[cmd]` — Steuerbefehl (nur bei `push`)
  - `[stream]` — Kontinuierlicher Datenstrom (optional bei `stream`)
...
push cnc machine commands [cmd]

write production data

stream machine status [stream]
```
...
Für alle Data Flows gilt die **unidirektionale Modellierung**. Bidirektionale Pfeile sind verboten. Für `pull [req]`-Flows wird eine **explizite Response** erzwungen:

| Request Flow | Response Flow | Bemerkung |
|-------------|---------------|------------|
| `pull … [req]` | `pull … [resp]` | Muss existieren; gleicher Verb, invertierte Richtung |

- **Gleicher Verb für beide Richtungen:** Request und Response verwenden denselben Verb (`pull`), weil die Interaktionsart beschrieben wird — nicht die Richtung.
- **Verbot bidirektionaler Pfeile:** Ein Request-Response-Paar wird immer durch zwei eindeutige, unidirektionale Flows modelliert.
...
#### Kommunikationsmuster und DataFlowProperties

Das Verb kodiert die primäre Semantik. Detailliertere Kommunikationsmuster (z. B. Pub/Sub, Message Queue) werden über die **DataFlowProperties** des DF-Elements beschrieben — nicht über das Label:

| Muster | Verb im Label | DataFlowProperties |
|--------|--------------|-------------------|
| Request/Response | `pull` | `direction: requestresponse` |
| Fire-and-Forget / Command | `push` | `direction: unidirectional` |
| Pub/Sub Event Delivery | `push` | `protocol: mqtt / amqp`, `direction: unidirectional` |
| Streaming (kontinuierlich) | `stream` | `frequency: continuous` |
| Data Store Write | `write` | `protocol: database / file` |

Diese Trennung hält das Label kompakt und gibt dem Analyzer-Tool präzise maschinenlesbare Metadaten für die STRIDE-Ableitung.
...
- **Automatisierung:** STRIDE-Mappings lassen sich direkt an Verb und Zieltyp koppeln — `write` impliziert Data-Store-Threats (Tampering, Information Disclosure), `push` impliziert Process-Threats (Spoofing, DoS).
- **Nachvollziehbarkeit:** Request/Response-Beziehungen sind durch Tool-Validierung prüfbar (TARAflow Checker).
- **Fehlerprävention:** Implizite oder bidirektionale Flows werden durch den Checker erkannt und gemeldet.
