...
**Phase 0 bestätigt (SmokeDetector-Fixture):** 0 von 19 Assets tragen eine safety-annotierte
`linkedDFDElements`-Kante; `hasSafetyAnnotation` ist überall `false`. Die 4 Fatality-Assets
(`DA-001…004`) haben `physicalImpact` nur über `physicalImpactSource: "manual"`. Der
Safety-Auto-Add in `syncFromDFD` feuert nie; `hasSafetyData()` ist `true` allein wegen der
manuellen Overrides. Die DFD-Safety-Projektion ist also tot — Safety überlebt rein analytisch.

### 3.5 Reale Store-Drift (belegt durch Fixture)

Phase 0 hat einen handfesteren Befund geliefert als erwartet: In der SmokeDetector-Fixture sind
`assets.linkedDFDElements` gegenüber `dfd.assets.linkedElements` **tatsächlich veraltet**. Beispiel
`DA-002`:

```
existing (assets):  displayId 'DF-8'  name 'push calibration command [req_resp]'
incoming (dfd):     displayId 'DF-9'  name 'pull calibration command [req_resp]'
                                                '...commands [req_resp]' → '...commands [cmd]'
```

Der DFD wurde editiert (Data-Flows umbenannt/umnummeriert), aber der Sync lief auf diesem Pfad
nicht nach — exakt der `HU-003`-Mechanismus, nur bei bestehenden Links statt bei neuen Assets.
`syncFromDFD` erkennt und repariert die Drift korrekt (`hasChanges: true` ist hier **richtig**).
Diese Fixture ist damit ein lebendes Exemplar der Zwei-Store-Divergenz und **direkte Evidenz für
Phase 2** (zentralisierter Sync) und den **Load-Backfill** (heilt bereits gedriftete Projekte
beim Öffnen).

> **Korrektur einer früheren Annahme:** Der `linkedElements`-Vergleich in `syncFromDFD` nutzt
> `JSON.stringify` und ist damit schlüssel-reihenfolge-sensitiv — ein **latenter Footgun**, aber
> *nicht* die Ursache der beobachteten Re-Sync-Änderungen (die sind echte Drift). Ein struktureller
> Vergleich ist optionales Härten in Phase 1, kein Bugfix.
