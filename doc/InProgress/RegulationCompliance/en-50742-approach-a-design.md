...
### 11.1 EL anchors — three cases, do not conflate

EL is authored/derived in the DFD and reaches a threat through exactly one
anchor, depending on the threat method **and the anchored element type**. These
are **independent paths**:

| Threat method | Anchor | Stored on threat as | EL source |
|---|---|---|---|
| per-element on an **Interface** | the linked **Interface** | `linkedElement` (`elementType: "Interface"`, `elementId` = element id) | Interface's own `exposureLevel` (derived by `deriveExposureLevels` from its TrustBoundary / ChipBoundary / PhysicalBoundary, manual override respected) |
| per-element on a **DataFlow** | the linked **DataFlow** | `linkedElement` (`elementType: "DataFlow"`, `elementId` = **connection** id) | the DataFlow's own `exposureLevel` (`deriveExposureLevels` resolves the TB-crossing via local `maxEL`) |
| per-interaction | the crossing **DataFlow** | `dataFlow` (`connectionId` = connection id) | the DataFlow's own `exposureLevel` |

**A DataFlow is BOTH a per-element STRIDE anchor and a per-interaction anchor —
this is the subtlety that broke the first adapter.** Per-element STRIDE runs over
every DFD element type it supports, and `STRIDE_PER_ELEMENT_TYPE.DataFlow =
[T, I, D]` (the per-element generator wraps each connection as a
`DFDElementReference { id: conn.id, type: "DataFlow" }`). So a data-flow threat
generated by the **per-element** pass carries its connection id in
`linkedElement.elementId` with `elementType: "DataFlow"` and a **null**
`dataFlow` — *not* in `dataFlow.connectionId`. Only the separate
**per-interaction** generator (trust-boundary crossings) populates `dataFlow`
with `interactionContext`.

Consequence for `resolveAnchorProperties` (`en50742-risk-calculation.ts`): a
`linkedElement.elementId` is looked up in `dfd.elements` **only when its
`elementType` is not `"DataFlow"`**; a DataFlow-typed linkedElement (and, as a
defensive fallback for older data lacking `elementType`, any linkedElement id
not found among the elements) is looked up in `dfd.connections`. draw.io cell
ids are unique across the whole graph, so the fallback can never mis-resolve.
...
> **Note (was a real bug, fixed):** the earlier two-row model listed the
> DataFlow only under *per-interaction*, so the adapter searched `dfd.elements`
> for every `linkedElement` id. Data-flow threats (the common case — every
> `[T, I, D]` on a DF) put a **connection** id there, which is never in
> `dfd.elements`, so EL silently stayed `0` for all of them. Confirmed on
> `Simple_Test_Project` (DF-3 = EL3, DF-4 = EL1 → both dropped to `0` in the
> Risk tab).
