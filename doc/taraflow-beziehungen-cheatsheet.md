...
> **PhysicalBoundary** ist ein connectables DFD-Element — repräsentiert räumlich-physische
> Zugriffsbarrieren (Gehäuse, Schaltschrank, Raum, Gebäude). Verbindet sich via `is_an`
> mit Physical oder Infrastructure Assets. DataFlows terminieren via Interface-Symbol auf der Boundary-Kante.
...
> **Kein DFD-Einstieg** — mit zwei Ausnahmen (Ebene-1-Beziehungen):
> 1. `ExternalEntity → damages → Physical Asset` für **Sabotage-Szenarien** (OT/Anlagensicherheit)
> 2. `PhysicalBoundary → [is_an | secures | accesses | damages] → Physical Asset` — PB ist selbst ein Physical Asset oder schützt eines

> Alle anderen Beziehungen sind **Asset-zu-Asset (Ebene 2)**.

> Enthält das Asset Software oder eine CPU → **System Asset**.

**Ebene-1-Beziehungen (Physical Assets):**

| Beziehung | Erlaubtes Element | Semantik |
|---|---|---|
| `damages` | ExternalEntity | Physische Zerstörung / Sabotage passiver Bauteile |
| `is_an` | PhysicalBoundary | Boundary ist Instanz des Physical Assets (z.B. Gehäuse = Physical Asset) |
| `secures` | PhysicalBoundary | Boundary schützt das Physical Asset im Innern |
| `accesses` | PhysicalBoundary | Angreifer-Pfad: durch die Boundary zum Physical Asset |
| `damages` | PhysicalBoundary | Physische Zerstörung der Barriere selbst |
...
> **PhysicalBoundary = räumlich-physische Zugriffsbarriere.** Connectable via Interface-Symbol auf der Boundary-Kante.

| Beziehung | Asset-Gruppe | Semantik |
|---|---|---|
| `is_an` | Physical / Infrastructure | Boundary ist Instanz des Assets (Schaltschrank = Physical Asset) |
| `secures` | Physical / Infrastructure | Boundary schützt das Asset im Innern |
| `accesses` | Physical | Angriffsvektor: physischer Zugang durch die Boundary |
| `damages` | Physical | Physische Zerstörung der Barriere |
| `powers` | Infrastructure | Infrastruktur versorgt die Boundary mit Strom |
| `depends_on` | System | System ist abhängig von der Existenz der physischen Barriere |

> **Keine Relationen zu:** Data, Function, Process, Service, Human — PB hat keinen Einfluss auf Daten, Funktionen oder Personen direkt.
