...
// Hazards have no DFD sync / export-import yet, so the bar is leaner:
// Add | Settings | … | count chip | validation chip | unsaved | Continue.
...
export const HazardToolbar = React.memo<HazardToolbarProps>(
  ({ isDirty, validation, hazardCount, onAdd, onOpenConfig, onProceed }) => {
    ...
    const getStatusText = (): string => {
      if (!validation) return t("status.inProgress", { defaultValue: "In Progress" });
      if (validation.isComplete) return t("status.complete", { defaultValue: "Complete" });
    ...
    return (
      ...
      >
        {/* Add hazard */}
        <Button startIcon={<AddIcon />} onClick={onAdd} size="small" variant="contained">
          {t("tabs.hazards.addHazard", { defaultValue: "Add Hazard" })}
        </Button>

        {/* Settings */}
        <Tooltip title={t("tabs.hazards.configuration", { defaultValue: "Hazard Settings" })}>
            ...
            validation && (validation.errors.length > 0 || validation.warnings.length > 0) ? (
              <Box sx={{ p: 0.5 }}>
                {validation.errors.map((err, i) => (
                  <Typography key={`e${i}`} variant="caption" display="block" color="rgba(255,180,180,1)">
                    • {err}
                  </Typography>
                ))}
                {validation.warnings.map((warn, i) => (
                  <Typography key={`w${i}`} variant="caption" display="block" color="rgba(255,220,100,1)">
                    • {warn}
                  </Typography>
                ))}
              </Box>
