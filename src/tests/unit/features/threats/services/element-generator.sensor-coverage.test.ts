...
describe("Sensor/Actuator coverage (RC-2)", () => {
  ...
  it("emits the full base table [S,T,D] for an unlinked orphan Sensor", () => {
    const tables = elementThreatGenerator.generateThreatsForProject(
      projectWithOrphanSensor(),
    );
    const cats = new Set(
    ...
    expect(cats).toEqual(new Set(["S", "T", "D"]));
  });
 
  it("category-level pipeline returns [S,T,D] for the orphan Sensor (regression anchor)", () => {
    ...
    expect(new Set(cats)).toEqual(new Set(["S", "T", "D"]));
  });
 
  it("uses authored Sensor templates with curated (≤3) controls, not the generic fallback", () => {
    // Sensor now has real element templates (S-003 / T-015 / D-007), so each
    // emitted threat carries an authored templateId and a curated, capped set of
    // mitigations — never the __generic-* fallback and never an empty shell.
    const tables = elementThreatGenerator.generateThreatsForProject(
      projectWithOrphanSensor(),
    );
    const sensorThreats = allThreats(tables).filter(
    ...
    for (const th of sensorThreats) {
      // Catalog-derived assertions only — no i18n bootstrap needed here
      // (threatDescription text is resolved from i18n and is covered elsewhere).
      expect(th.proposedMitigations.length).toBeGreaterThan(0);
      expect(th.proposedMitigations.length).toBeLessThanOrEqual(3);
      expect(th.proposedVerifications.length).toBeLessThanOrEqual(3);
      // authored template id (e.g. S-003 / T-015 / D-007), not the fallback
      const templateId = th.templateId ?? "";
      expect(templateId).toMatch(/^[STD]-\d+$/);
      expect(templateId.startsWith("__generic")).toBe(false);
    }
  });
});
